...
# Get all accounts - response is { data: [...], total }
$resp = Invoke-RestMethod -Uri "http://localhost:1930/api/accounts" -Method GET -Headers $headers
$accounts = $resp.data
Write-Host "Found $($accounts.Count) existing accounts"

# Delete all
if ($accounts.Count -gt 0) {
    $ids = $accounts | ForEach-Object { [int]$_.id }
    $deleteBody = @{ ids = $ids } | ConvertTo-Json
    $result = Invoke-RestMethod -Uri "http://localhost:1930/api/accounts/bulk-delete" -Method POST -Headers $headers -Body $deleteBody
    Write-Host "Deleted $($result.deleted) accounts"
}
...
$body = @{ accounts = $newAccounts } | ConvertTo-Json -Depth 3
$result = Invoke-RestMethod -Uri "http://localhost:1930/api/accounts/bulk" -Method POST -Headers $headers -Body $body
Write-Host "`nImport done: $($result.success) success, $($result.failed) failed"
